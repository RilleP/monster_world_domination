	...
	return {
		"gui": {
			js_draw_text: (ptr, len, font_size, x, y, align_x, align_y, r,g,b,a) => {
				...
				//ctx.fillStyle = `rgba(${color&0xff}, ${255}, ${100}, ${100})`;
		...
		"audio": {
			play_sound: (ptr, len, volume) => {
				var audio = new Audio(wasmMemoryInterface.loadString(ptr, len));
...
async function runWasm(wasmPath, consoleElement, extraForeignImports) {
	...
	window.gui_canvas = document.getElementById("guicanvas");
	window.gui_ctx = window.gui_canvas.getContext("2d");
				

	exports._start();

	music = new Audio("res/dominatione.ogg");
	...
	if (exports.step) {
		...
		const step = (currTimeStamp) => {
			...
			if(glctx != undefined) {
				glctx.canvas.width = window.innerWidth;
				glctx.canvas.height = window.innerHeight;
			}
			if(window.gui_canvas != null) {
				window.gui_canvas.width = glctx.canvas.width;
				window.gui_canvas.height = glctx.canvas.height;
			}			
